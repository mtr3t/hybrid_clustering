...
def plotter(Y, kmeans_labels, nmi, prob_type, prob_title, sigma, printer=False, save_figure=False, axis_hold=False):
    ...
    text = '../07_figures/04_hybrid_clustering/' + prob_type + '_solve_' + prob_title.replace("../05_", '').replace("s/", '_').replace(".", '_') + '_sigma_gamma_' + str(sigma).replace(".", '_').replace(", ", '_') + '_nmi_' + str(nmi).replace(".", '_') + '.png'
